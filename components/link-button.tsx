...
const variantClasses: Record<NonNullable<LinkButtonProps["variant"]>, string> = {
  primary:
    "group border border-accent/10 bg-[linear-gradient(135deg,#0f766e,#115e59)] text-white shadow-lift ring-1 ring-white/10 shadow-[0_16px_34px_rgba(15,118,110,0.18)] hover:-translate-y-0.5 hover:shadow-[0_20px_42px_rgba(15,118,110,0.24)]",
  secondary:
    ...
    >
      ...
      <span
        ...
        className={`text-base transition-transform duration-300 group-hover:translate-x-0.5 ${
          variant === "primary"
            ? "inline-flex h-7 w-7 items-center justify-center rounded-full bg-white/12"
            : ""
        }`}
      >
        →
