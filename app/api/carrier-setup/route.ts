  ...
  try {
    ...
    if (totalUploadBytes > maxCarrierUploadBundleBytes) {
      return NextResponse.json(
        {
          message:
            "The total upload package is too large for secure delivery. Please keep the combined file size under 20 MB or submit fewer files at once.",
        },
        { status: 413 },
      );
    }
    ...
    console.error("Carrier setup request crashed before response", error);
